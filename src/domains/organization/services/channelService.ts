...
const channelService = {
  ...
  getChannels: async (): Promise<Channel[]> => {
    try {
      const channelsUrl = API_PATHS.channels.root();
  ...
  createChannel: async (code: string, name: string): Promise<Channel> => {
    try {
      const { data } = await axiosInstance.post(API_PATHS.channels.root(), {
        code,
        name,
        is_active: true
      })
  ...
  updateChannel: async (
    ...
    try {
      const { data } = await axiosInstance.patch(API_PATHS.channels.byId(id), changes)
  ...
  deleteChannel: async (id: number): Promise<void> => {
    try {
      await axiosInstance.delete(API_PATHS.channels.byId(id))
